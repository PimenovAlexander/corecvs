...
void TestbedMainWindow::connectActions()
{
    ...
    connect(mUi->actionDistorrionCorrector, SIGNAL(triggered()), mDistortionWidget, SLOT(show()));
...
void TestbedMainWindow::loadImage(void)
{
    ...
    G12Buffer *distInput = mImage->toG12Buffer();
    mDistortionWidget->setBuffer(distInput);
    delete_safe(distInput);
