...
int main(int argc, char **argv)
{
    ...
    for(int l = 0; l < 1000; ++l)
        for (size_t i = 0; i < 99; ++i)
            for (size_t j = 0; j < 99; ++j)
            {
                isInteriorConvexPol(a[i][j], DegeneratePol); //isInteriorROConvexPolBinSearch(a[i][j], DegeneratePol); for some reason runtimes are different depending on order
    ...
    for(int l = 0; l < 1000; ++l)
        for (size_t i = 0; i < 99; ++i)
            for (size_t j = 0; j < 99; ++j)
            {
                isInteriorROConvexPolBinSearch(a[i][j], DegeneratePol); // isInteriorConvexPol(a[i][j], DegeneratePol);
            }
    t1 = clock() - t1;
    ...
    for(int l = 0; l < 100000; ++l)
        ...
        {
            isInteriorConvexPol(DegeneratePol.getPoint(i), DegeneratePol);
        }
    t1 = clock() - t1;
    cout << "runtime without BS: " <<  t1 << endl;
    ...
    for(int l = 0; l < 100000; ++l)
        for (size_t i = 0; i < 8; ++i)
        {
            isInteriorROConvexPolBinSearch(DegeneratePol.getPoint(i), DegeneratePol);
        }
    t1 = clock() - t1;
    cout << "runtime with BS: " <<  t1 << endl;
