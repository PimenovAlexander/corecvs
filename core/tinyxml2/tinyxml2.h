...
public:
    ...
    virtual void* Alloc() {
        if ( !_root ) {
            ...
            block->chunk[SIZECOUNT-1].next = 0;
            _root = block->chunk;
        }
        void* result = _root;
        _root = _root->next;
    ...
    enum { SIZECOUNT = 1024/SIZE }; // Some compilers do not accept to use COUNT in private part if COUNT is private
